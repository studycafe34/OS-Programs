...
void findCompletionTime(struct Process processes[], int n) {
    processes[0].completion = processes[0].arrival + processes[0].burst; // Completion time for first process
    for (int i = 1; i < n; i++) {
...
int main() {
    ...
    // Sort processes by arrival time (if needed) to handle earlier processes first
    for (int i = 0; i < n - 1; i++) {
        for (int j = i + 1; j < n; j++) {
